...
function get_reselling_ticket_list_info(cb) {
    ...
    var sqlquery = "SELECT b.ticket_owner_index, b.ticket_id FROM bidding b";
...
function alert_original_ticket_owner(ticket_owner_id, ticket_id, current_price,bidder_index) {
            var gig_venue = network.get_ticket_info_by_id(bidder_index, ticket_id).gig_venue;
            var gig_name =  network.get_ticket_info_by_id(bidder_index, ticket_id).gig_name;
            var gig_time =  network.get_ticket_info_by_id(bidder_index, ticket_id).gig_time;
            var section_id= network.get_ticket_info_by_id(bidder_index, ticket_id).section_id;
            var row_id=network.get_ticket_info_by_id(bidder_index, ticket_id).row_id;
            var seat_id=network.get_ticket_info_by_id(bidder_index, ticket_id).seat_id;
...
function change_ticket_owner(bidding_index,bidder_index, ticket_owner_id, ticket_id, current_price, cb) {
    if (bidder_index != -1) {
        if (network.update_ticket_owner(bidder_index, ticket_id) == true) {
            ...
            if (alert_original_ticket_owner(ticket_owner_id, ticket_id, current_price, bidder_index) == true) {
...
//끝난 비딩 체크하기
    function check_bidding_over(cb) {
        ...
        connection.query(sqlquery, function (err, rows) {
            if (!err) {
                bidding_list=rows;
                for (var i = 0; i < bidding_list.size(); i++) {
                    if (right_now_time >= bidding_list[i].end_time || bidding_list[i].max_price === bidding_list[i].current_price) {
                        console.log("bidding over!");
                        change_ticket_owner(bidding_list[i].bidding_index, bidding_list[i].bidder_index, bidding_list[i].ticket_owner_index, bidding_list[i].ticket_id, bidding_list[i].current_price);
                    }
                }
            }
            else
